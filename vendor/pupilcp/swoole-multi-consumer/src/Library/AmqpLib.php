...
class AmqpLib
{
     ...
     * 一个进程定时检测tcp连接上是否有数据发送（这里的发送是指rabbitmq发送数据给客户端），如果一段时间内没有数据发送给客户端，则发送一个心跳包给客户端，然后循环进行下一次检测；
     * 另一个进程定时检测tcp连接上是否有数据的接收，如果一段时间内没有收到任何数据，则判定为心跳超时，最终会关闭tcp连接。
     * @throws \Exception
     */
    private function __construct($host, $port, $user, $pass, $vhost, $exchange, $timeout = 5, $heartbeat = 30)
    {
        $this->host = $host;
        $this->port = $port;
        $this->user = $user;
